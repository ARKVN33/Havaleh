    ...
    public class BackupRestore
    {
        ...
        public void BackUpDb()
        {
            ...
            try
            {
                ...
                sqlConnection.Open();
                var backupQuery = $@"BACKUP DATABASE dbHavaleh TO  DISK = N'{BackupPath}' WITH NOFORMAT, NOINIT,  NAME = N'dbHavaleh-Full Database Backup', SKIP, NOREWIND, NOUNLOAD,  STATS = 10";
        ...
        public void RestoreDb()
        {
            ...
            var currentDatabase = server.Databases[@"dbHavaleh"];

            if (currentDatabase != null)
            {
                server.KillAllProcesses(@"dbHavaleh");
            }
            var appDataDirectory = Globals.MyAppData;
            var appDbDirectory = Path.Combine(appDataDirectory, @"DATABASE");
            var query =
                $@"RESTORE DATABASE[dbHavaleh] FROM DISK = N'{BackupPath}' WITH REPLACE, MOVE N'dbHavaleh' TO N'{
                        appDbDirectory
                    }\dbHavaleh.mdf',  MOVE N'dbHavaleh_log' TO N'{appDbDirectory}\dbHavaleh_LOG.ldf' ,  NOUNLOAD,  STATS = 5";
        ...
        private static void CheckBackupFile(string backupPath)
        {
            var sqlConnection = new SqlConnection
            {
                ConnectionString = @"Data Source=(LocalDb)\v12.0;Database=dbHavaleh;Integrated Security=True;Connect Timeout=30"
            };
            sqlConnection.Open();
