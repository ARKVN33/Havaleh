...
namespace DAL.Class
{
    public class HavalehDbConfiguration
    {
        ...
        public void Configurate()
        {
            ...
            var dbName = _directoryPath + @"\dbHavaleh.mdf";
            var dbLog = _directoryPath + @"\dbHavaleh_LOG.ldf";
            ...
            {
                da.Fill(dt);
                if (dt.Rows.Cast<DataRow>().Any(item => item["DATABASE_NAME"].ToString() == "dbHavaleh"))
                    exists = true;
            ...
            if (!File.Exists(_directoryPath + @"\dbHavaleh.mdf"))
            {
